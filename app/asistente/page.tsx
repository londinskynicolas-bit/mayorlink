...
export default function Asistente() {
  ...
  const enviar = () => {
    ...
    setMensajes(prev => [...prev, { rol: "user", texto: pregunta }, { rol: "assistant", texto: respuesta }]);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-black text-white px-4 md:px-6 py-6 md:py-8">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center gap-3 md:gap-4">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-500 rounded-2xl flex items-center justify-center text-xl md:text-2xl flex-shrink-0">🤖</div>
            <div className="min-w-0">
              <h1 className="text-lg md:text-2xl font-black">Asistente Comercial</h1>
              <p className="text-gray-400 text-xs md:text-sm">Experto en MayorLink</p>
            </div>
            <div className="ml-auto flex items-center gap-2 flex-shrink-0">
              <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
              <span className="text-emerald-400 text-xs font-bold hidden sm:block">En linea</span>
            </div>
          </div>
        </div>
      </div>

      <div className="flex-1 max-w-3xl w-full mx-auto px-4 md:px-6 py-4 md:py-6 flex flex-col gap-4 overflow-y-auto">
        {mensajes.map((m, i) => (
          <div key={i} className={"flex " + (m.rol === "user" ? "justify-end" : "justify-start")}>
            {m.rol === "assistant" && (
              <div className="w-7 h-7 md:w-8 md:h-8 bg-emerald-500 rounded-xl flex items-center justify-center text-sm mr-2 md:mr-3 flex-shrink-0 mt-1">🤖</div>
            )}
            <div className={"max-w-[85%] md:max-w-lg px-4 md:px-5 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap " + (m.rol === "user" ? "bg-black text-white rounded-tr-sm" : "bg-white border-2 border-gray-100 text-gray-800 rounded-tl-sm")}>
              {m.texto}
            </div>
          </div>
        ))}
      ...
      <div className="bg-white border-t-2 border-gray-100 px-4 md:px-6 py-3 md:py-4">
        <div className="max-w-3xl mx-auto">
          <div className="flex gap-2 mb-3 flex-wrap">
            {["Como publico mi empresa?", "Como busco proveedores?", "Es gratis?"].map((s) => (
          ...
          <div className="flex gap-2 md:gap-3">
            ...
            />
            <button onClick={enviar} disabled={!input.trim()} className="bg-emerald-500 hover:bg-emerald-400 text-black font-black px-5 md:px-6 py-3 rounded-xl transition-colors disabled:opacity-50">
              Enviar
            </button>
          </div>
        </div>
