...
import { useProvincias, useMunicipios } from "../../hooks/useLocalidades";
...
export default function RegistroProveedor() {
  ...
  const { municipios } = useMunicipios(provinciaSeleccionada);
          ...
          {paso === 1 && (
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <select value={provinciaSeleccionada} onChange={(e) => { setProvinciaSeleccionada(e.target.value); setMunicipioSeleccionado(""); }} className="w-full border-2 border-gray-200 rounded-xl px-3 py-3 text-sm focus:outline-none focus:border-black">
              ...
              {provinciaSeleccionada && (
                <div>
                  <label className="text-sm font-bold text-gray-700 block mb-1">Municipio / Ciudad</label>
                  <select value={municipioSeleccionado} onChange={(e) => setMunicipioSeleccionado(e.target.value)} className="w-full border-2 border-gray-200 rounded-xl px-3 py-3 text-sm focus:outline-none focus:border-black">
